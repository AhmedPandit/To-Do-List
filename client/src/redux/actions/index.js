...
export const getAllTodos = () => async (dispatch) => {
    try {
        ...
        dispatch({ type: GETALL_TODO , payload: res.data.itemlist });
    } catch (error) {
...
export const updateTodo = (id, data) => async (dispatch) => {
    try {
        const res = await axios.put(`${API_URL}/todos/updateitem/${id}`, { data });
...
export const deleteTodo = (id) => async (dispatch) => {
    try {
        const res = await axios.delete(`${API_URL}/todos/deleteitem/${id}`);
